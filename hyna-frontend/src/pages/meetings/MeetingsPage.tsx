...
import { Plus, Clock, Video, Users, Calendar } from 'lucide-react';
import { Button, Avatar, AvatarGroup, Badge, Modal, Input, Textarea, Select, EmptyState } from '@/components/ui';
import { cn, formatDate, formatTime, getStatusColor } from '@/lib/utils';
import { useAuthStore } from '@/stores';
import { mockMeetings, getUserById, mockUsers } from '@/mock/data';
...
export function MeetingsPage() {
  ...
  const meetings = currentRole === 'member'
    ? mockMeetings.filter(m => m.participantIds.includes(currentUser?.id || ''))
    : mockMeetings;
  ...
  return (
    <div className="page-container">
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1 p-1 rounded-lg bg-[var(--color-muted)]">
            {(['upcoming', 'past', 'all'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} className={cn('px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-colors', filter === f ? 'bg-[var(--color-card)] shadow-sm' : 'text-[var(--color-muted-foreground)]')}>{f}</button>
            ))}
          </div>
          {currentRole !== 'member' && <Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4 mr-1" /> New Meeting</Button>}
        </div>
      </div>
              ...
              >
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-[var(--color-border)]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[var(--color-muted-foreground)]">{host?.name}</span>
                  </div>
                  {meeting.meetingLink && (
                    <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); toast.info('Joining meeting...'); }}>
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create Meeting" size="lg"
        footer={<><Button variant="outline" onClick={() => setShowCreate(false)}>Cancel</Button><Button onClick={() => { setShowCreate(false); toast.success('Meeting created!'); }}>Create Meeting</Button></>}>
        <div className="space-y-4">
          <Input label="Meeting Title" placeholder="e.g., Weekly Team Meeting" />
          <Textarea label="Description" placeholder="Meeting agenda..." rows={3} />
          <div className="grid grid-cols-2 gap-4">
            <Input label="Date" type="date" />
            <Select label="Type" options={[{ value: 'team', label: 'Team' }, { value: 'standup', label: 'Standup' }, { value: 'review', label: 'Review' }, { value: 'planning', label: 'Planning' }, { value: 'one-on-one', label: '1:1' }]} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Input label="Start Time" type="time" />
            <Input label="End Time" type="time" />
          </div>
          <Input label="Meeting Link" placeholder="https://meet.hynastudio.com/..." />
        </div>
      </Modal>
    </div>
  );
